...
async function pay_handler(bot, player_id, amount, type, is_bet) {
    ...
    if (type == 'e' || type == 'emerald') {
        ...
        return new Promise(async resolve => {
            await Promise.race([negative_Promise, no_emerald_Promise, success_Promise, not_same_place_Promise, wait_Promise, timeout_Promise]).then(async (string) => {
        ...
        await chat(bot, `/cointrans ${player_id} ${Math.floor(amount * config.bet.codds)}`)
